...
for file in files:
    with open(os.path.join(path, file)) as f:
        ...
        df_annotator["surface"] = [
            result[0]["value"]["choices"][0] for result in annotations.result
        ]
        ...
        df_annotator["smoothness"] = [
            result[1]["value"]["choices"][0] if (result[1]["value"]["choices"]) else ""
            for result in annotations.result
        ]
        df_annotator["res2"] = [
            result[2]["value"]["choices"][0] if (result[2]["value"]["choices"]) else ""
            for result in annotations.result
        ]
        df_annotator["res3"] = [
            result[3]["value"]["choices"][0] if (len(result) > 3) else ""
            for result in annotations.result
        ]
    ...
    df.loc[
        (df.res2.isin(nostreet_values)) | (df.res3.isin(nostreet_values)), "nostreet"
    ] = "nostreet"
    df.loc[
        (df.res2.isin == unsure_revise_value) | (df.res3 == unsure_revise_value),
        "nostreet",
    ] = "revise"
...
grouped_surface = (
    df[df.image_id.isin(image_ids)].groupby(["image_id", "surface"]).size()
)
...
grouped_smoothness = (
    df[df.image_id.isin(image_ids)].groupby(["image_id", "smoothness"]).size()
)
round(
    100 * len(grouped_smoothness[grouped_smoothness == 3]) / len(grouped_smoothness), 2
)
# 30%
...
for i, j in [(1, 3), (1, 4), (3, 4)]:
    rater1 = df[(df.image_id.isin(image_ids)) & (df.annotator == i)].sort_values(
        by=["image_id"]
    )
    rater2 = df[(df.image_id.isin(image_ids)) & (df.annotator == j)].sort_values(
        by=["image_id"]
    )
...
rater1 = df[(df.image_id.isin(image_ids)) & (df.annotator == 1)].sort_values(
    by=["image_id"]
)
rater2 = df[(df.image_id.isin(image_ids)) & (df.annotator == 3)].sort_values(
    by=["image_id"]
)
rater3 = df[(df.image_id.isin(image_ids)) & (df.annotator == 4)].sort_values(
    by=["image_id"]
)
krippendorfs_alpha_surf = kd.alpha(
    np.array(
        [rater1.surface.tolist(), rater2.surface.tolist(), rater3.surface.tolist()]
    ),
    level_of_measurement="nominal",
)
krippendorfs_alpha_smooth = kd.alpha(
    np.array(
        [
            rater1.smoothness.tolist(),
            rater2.smoothness.tolist(),
            rater3.smoothness.tolist(),
        ]
    ),
    level_of_measurement="nominal",
)
