...
pred_files = {
    const.PAVING_STONES: "all_train_paving_stones_prediction-V11_annotated_paving_stones-20240417_094244.csv"
}
...
model_pred_path = os.path.join(
    ds_metadata_path, "model_predictions", pred_files[const.PAVING_STONES]
)
...
misclassified = df[
    ((df.quality_float_true - df.Prediction) > 1 & (df.surface_true.notna()))
]
...
gpt_imgs = pd.read_csv(
    os.path.join(
        config.cloud_image_folder.split("/mapillary_images")[0],
        "labeling",
        "gpt_experiments",
        "images_pre_experiment_paving_stones.csv",
    ),
    dtype={"image_id": str},
)
