...
def test_entity(row, img_folder):
    ...
    entity["predictions"] = [{"result": result}]
    return entity
...
def filter_by_model_prediction(metadata, chunk_id):
    model_pred_path = os.path.join(
        config.model_prediction_path.format(
            (config.ds_version + "_c" + str(chunk_id)), config.ds_version, chunk_id
        ),
    ...
    df = metadata.set_index("id").join(model_prediction.set_index("Image"), how="left")
    df["combined_prediction"] = "no prediction"
    df.loc[df.Level_0 == df.surface_clean, "combined_prediction"] = df[
        df.Level_0 == df.surface_clean
    ].surface_clean

    with open(
        config.chunk_filtered_img_ids_path.format(config.ds_version, chunk_id, "txt"),
        "w",
    ) as file:
    ...
    if os.path.exists(
        config.manual_added_images.format(config.ds_version, chunk_id, "txt")
    ):
        with open(
            config.manual_added_images.format(config.ds_version, chunk_id, "txt"), "r"
        ) as file:
    ...
    for chunk_id in chunk_ids:
        if chunk_id == 0:
            ...
            create_labelstudio_input_file(
                metadata[metadata.id.isin(chunk_img_ids)].sort_values(
            ...
            with open(
                config.chunk_img_ids_path.format(config.ds_version, chunk_id, "txt"),
                "r",
            ) as file:
                chunk_img_ids = file.read().splitlines()
            ...
            if not ((config.ds_version == "v5") & (np.isin(chunk_id, [6, 8]))):
                filtered_chunk_img_ids = filter_by_model_prediction(
                    chunk_metadata, chunk_id
                )
                chunk_metadata = metadata[metadata.id.isin(filtered_chunk_img_ids)]

            md_grouped = chunk_metadata.groupby(["surface_clean", "smoothness_clean"])
            ...
            for i in range(0, config.n_annotators):
                ...
                for label in md_grouped.size().index:
                    ...
                    imgids_ann += (
                        chunk_metadata[
                            (chunk_metadata.surface_clean == label[0])
                            & (chunk_metadata.smoothness_clean == label[1])
                        ]
                ...
                with open(
                    config.annotator_ids_path.format(
                        config.ds_version, chunk_id, i, "txt"
                    ),
                    "w",
                ) as file:
                ...
                create_labelstudio_input_file(
                    metadata[metadata.id.isin(imgids_ann)].sort_values(
                        ["surface_clean", "smoothness_clean"]
                    ),
                    False,
                    config.annotator_ids_path.format(
