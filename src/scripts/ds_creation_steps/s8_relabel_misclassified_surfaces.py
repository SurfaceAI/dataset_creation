...
model_pred_path = os.path.join(ds_metadata_path, "model_predictions/all_train_effnet_surface_quality_prediction-V9_annotated-20240318_130115.csv")
...
pred = pred.loc[idx] 
pred.rename(columns={"Image": "image_id", "Prediction": "class_prob", "Level_0": "surface_pred"}, inplace=True)
...
df = pred.set_index("image_id").join(annot[["image_id", "surface_true", "quality_label_true", "input_version"]].set_index("image_id"), how="left")
...
create_labelstudio_input_file(metadata, is_testdata=False, output_path=os.path.join(ds_metadata_path, "relabel_misclassified.json"), test_city=None)