...
def get_train_tiles_metadata(write_to_db=True):
        ...
        utils.query_and_write_img_metadata(tiles, config.train_tiles_metadata_path.format(config.ds_version))
        ...
        if write_to_db:
            ...
            with conn.cursor(cursor_factory=DictCursor) as cursor:
                ...
                cursor.execute(
                    sql.SQL(
                        query.format(
                            table_name=f"{config.ds_version}_mapillary_meta", absolute_path=absolute_path
                        )
                    )
                )
                conn.commit()
