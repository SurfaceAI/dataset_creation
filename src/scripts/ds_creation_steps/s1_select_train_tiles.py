...
def select_train_tiles(exclude_cities, use_osm_tags=True, tag_combiantions=None, remove_old_tiles=True,random_state=1):
    ...
    if not os.path.exists(config.germany_tiles_path):
        print(f"Mapillary image counts for germany ({config.germany_tiles_path}) does not exist. Creating it now. This might take a few hours.")
    ...
    if remove_old_tiles:
        ...
        for ds_version in old_ds_versions:
            old_tiles = pd.read_csv(config.train_tiles_selection_path.format(ds_version))
            old_tiles["tile_id"] = (
    ...
    if use_osm_tags:
        ...
        for surface in surfaces:
            ...
            for smoothness in smoothnesses:
                ...
                if not os.path.exists(tif_path):
                    print(f"OSM tag count tif does not exist for {surface} {smoothness}. Creating it now.")
                ...
                n_tiles = config.tile_sample_size if config.tile_sample_size < len(osm_tiles) else len(osm_tiles)
                random.seed(1)
                sampled_tile_ids = random.sample(
                    osm_tiles.tile_id.tolist(), n_tiles
                )
        ...
        sampled_tiles = tiles[tiles.image_count > config.min_images].sample(config.ds_tile_count, random_state=random_state)


    os.makedirs(Path(config.train_tiles_selection_path.format(config.ds_version)).parent, exist_ok=True)
    sampled_tiles.to_csv(config.train_tiles_selection_path.format(config.ds_version), index=False)
...
if __name__=="__main__":
    ...
    # V200
    select_train_tiles(test_cities, use_osm_tags=False, remove_old_tiles=False,random_state=123)