...
def download_training_images(chunk_ids=None, n_per_chunk=100, copy_existing=False):
    ...
    if copy_existing:
        ...
        destination_folder_path = config.chunk_image_folder.format(
            config.ds_version, chunk_id
        )
        ...
        for i in range(len(df)):
            ...
            origin_path = os.path.join(
                origin_folder_path,
                f"{config.ds_version}_c{img_chunk}",
                "unsorted_images",
                f"{img_id}.jpg",
            )
        ...
        if not n_per_chunk is None:
            ...
            for chunk_id in chunk_ids:
                if chunk_id == 0:
                    ...
                    with open(
                        config.chunk_img_ids_path.format(
                            config.ds_version, chunk_id, "txt"
                        ),
                        "r",
                    ) as file:
                        img_ids_to_download += [
