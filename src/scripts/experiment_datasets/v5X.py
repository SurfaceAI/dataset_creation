    ...
    for smoothness in config.surf_smooth_comb[surface]:
        ...
        for sequence_id in tqdm(sequence_ids.index):
            ...
            n_to_sample = ( n_to_sample 
                            if n_to_sample <= len(metadata_class[metadata_class.sequence_id == sequence_id]) 
            ...
            if n_to_sample < 0:
                ...
                temp = (
                        metadata_class[metadata_class.sequence_id == sequence_id] 
        ...
        for tile_id in tqdm(tile_ids.index):
            ...
            n_to_sample = n_to_sample if n_to_sample <= len(sequence_selection[sequence_selection.tile_id == tile_id] ) else len(sequence_selection[sequence_selection.tile_id == tile_id])
            temp = (
...
metadata_v5x_selection.to_csv(config.train_image_selection_metadata_path.format("v5x"), index=False)