...
def sample_decoded_geometry():
    geom = [
        [[217, 2690], [227, 2695], [240, 2695], [244, 2690], [246, 2687], [246, 2682], [242, 2679], [238, 2673], [221, 2676], [219, 2682], [213, 2684], [217, 2690]],
        [[270, 2657], [270, 2665], [274, 2671], [285, 2671], [291, 2660], [285, 2646], [274, 2646], [270, 2657]],
        [[295, 2635], [299, 2643], [305, 2646], [315, 2643], [317, 2632], [317, 2619], [309, 2619], [299, 2621], [295, 2635]],
        [[317, 2619], [330, 2619], [340, 2608], [340, 2600], [352, 2589], [354, 2586], [356, 2583], [358, 2570], [365, 2564], [369, 2545], [373, 2542], [379, 2526], [389, 2474], [389, 2458], [391, 2359], [393, 2305], [397, 2165], [395, 1963], [391, 1966], [389, 2059], [389, 2073], [389, 2092], [381, 2307], [381, 2392], [373, 2455], [356, 2526], [352, 2537], [346, 2542], [342, 2561], [334, 2570], [332, 2583], [319, 2597], [317, 2608], [317, 2619]],
    ]
    return geom

class TestDecodeDetectionGeometry:

    def test_splitted_geometry(self, sample_road_types, sample_detections, sample_decoded_geometry):
        detection = mapillary_detections.filter_detections(sample_detections, 'a', types=sample_road_types)[0]
...
def sample_decoded_detections():
    return [[[0, 0], [(100/200*4096), 0], [0, (50/100*4096)], [0, 0]], [[(200/200*4096), 0], [(200/200*4096), (5.5/100*4096)], [(194.5/200*4096), 0], [(200/200*4096), 0]]]

@pytest.fixture(scope='class')
def sample_rescaled_detections():
    return np.asarray([[[0.0, 0.0], [100.0, 0.0], [0.0, 50.0], [0.0, 0.0]], [[200.0, 0.0], [200.0, 5.5], [194.5, 0.0], [200.0, 0.0]]])

class TestRescaleDetectionToImageSize:

    def test_simple_resizing(self, sample_decoded_detections, sample_rescaled_detections):
        
        size = (200, 100)

        polygon_transformed = mapillary_detections.rescale_detection_to_image_size(sample_decoded_detections[0], size)
...
class TestProcessDetections:
    ...
    def test_simple_processing(self, mock_decode_detection_geometry, sample_detection_names, sample_rescaled_detections):


        processed_detections = mapillary_detections.process_detections(sample_detection_names, (200, 100))
...
class TestDrawPolygon:
    ...
    def test_simple_draw(self, mock_decode_detection_geometry, sample_rescaled_detections):
...
class TestHasRoadType:
    ...
    def test_existing_type(self, sample_road_types, sample_detections):
        assert mapillary_detections.has_road_type(sample_detections, 'b', types=sample_road_types)

    def test_nonexisting_type(self, sample_road_types, sample_detections):
        assert mapillary_detections.has_road_type(sample_detections, 'c', types=sample_road_types) == False
...
class TestFilterDetections:
    ...
    def test_existing_type(self, sample_road_types, sample_detections):
        detection_list = mapillary_detections.filter_detections(sample_detections, 'b', types=sample_road_types)
        assert detection_list == ["GiB4AgoGbXB5LW9yKIAgEhEYAwgBIgsJ9CuaJRIcAAAMDw==",
                                  "GiZ4AgoGbXB5LW9yKIAgEhcYAwgBIhEJ0iuyJCoGFQgABAwDAAMKDw=="]
        
    def test_nonexisting_type(self, sample_road_types, sample_detections):
        detection_list = mapillary_detections.filter_detections(sample_detections, 'c', types=sample_road_types)
...
class TestCropAllAreasImage:
    ...
    def test_two_simple_polygons(self, mock_decode_detection_geometry, sample_image, sample_crop_image_large, sample_detection_names):

        image_crop = mapillary_detections.crop_all_areas_image(sample_image, sample_detection_names)
...
class TestCropLargestAreaImage:
    ...
    def test_two_simple_polygons(self, mock_decode_detection_geometry, sample_image, sample_crop_image_small, sample_detection_names):

        image_crop = mapillary_detections.crop_largest_area_image(sample_image, sample_detection_names)
...
class TestMaskImage:
    ...
    def test_two_simple_polygons(self, mock_decode_detection_geometry, sample_detection_names, sample_image):

        image_masked = mapillary_detections.mask_image(sample_image, sample_detection_names, (255, 255, 255))
