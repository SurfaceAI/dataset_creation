...
def decode_detection_geometry(detection: str):
    ...
    features = get_value_from_dict('features', get_value_from_dict('mpy-or', decoded_geometry))
    ...
    for feature in features:
        geometry.extend(get_value_from_dict('coordinates', get_value_from_dict('geometry', feature)))
...
def has_road_type(detections: dict, road_type, types=road_types):
    ...
    return any(get_value_from_dict('value', detection) in types[road_type]['detections'] for detection in data)
...
def filter_detections(detections: dict, road_type, types=road_types):
    ...
    return [get_value_from_dict('geometry', detection) for detection in data if get_value_from_dict('value', detection) in types[road_type]['detections']]
...
def crop_all_areas_image(image: Image, detection_list):
    ...
    polygons = [convert_to_polygon(detection) for detection in process_detections(detection_list, image.size)]
...
def crop_largest_area_image(image: Image, detection_list):
    ...
    polygon = max(polygons, key=lambda polygon: calculate_polygon_area(polygon), default=None)
    bbox = generate_polygon_bbox(polygon)
    image_crop = crop_image_by_bbox(image, bbox)
...
def crop_image_by_road_type_all_areas(image_data, road_type, image_size='thumb_1024_url', threshold=0.05):
...
def crop_image_by_road_type_largest_area(image_data, road_type, image_size='thumb_1024_url', threshold=0.05):
...
def mask_image_by_road_type(image_data, road_type, image_size='thumb_1024_url', threshold=0.05):
