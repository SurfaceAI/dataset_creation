...
cities = ["heilbronn", "lueneburg", "dresden"]
# citites = ["cologne", "muenchen", "dresden", "heilbronn", "lueneburg""]
cities = ["muenchen"]
...
for city in cities:
    ...
    if not os.path.exists(config.test_small_raster_template.format(city)):
        ...
        rf.create_raster(int(xmin), int(xmax), int(ymin), int(ymax), "epsg:3035", config.test_small_raster_template.format(city), resolution=100)
...
if create_training_tiles:
    ...
    #   - exclude all tiles of test data cities
    test_tiles = pd.read_csv(config.test_city_tiles_path.format(city))
    ...
    # TODO: only quick fix, to align both rasters
    #osm_tags = osm_tags[osm_tags.shape[0]-mapillary_counts.shape[0]:, osm_tags.shape[1]-mapillary_counts.shape[1]:]
    
    # TODO: get tile ids from raster    
    ...
    metadata = (metadata
                .groupby("sequence_id")
                .sample(1, random_state=1, replace=True,))
    # 44.733 sequences
