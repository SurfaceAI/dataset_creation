...
sample_size = 100
metadata = metadata.groupby(["surface_clean", "smoothness"]).sample(
...
for i in range(4):
    for surface in metadata.surface_clean.unique():
        for smoothness in metadata.smoothness.unique():
            ...
            if i == 0:
                current_third = metadata[
                    (metadata["surface_clean"] == surface)
                    & (metadata["smoothness"] == smoothness)
                ][0:classified_by_everyone]
            else:
                current_third = metadata[
                    (metadata["surface_clean"] == surface)
                    & (metadata["smoothness"] == smoothness)
                ][
                    classified_by_everyone
                    + ((i - 1) * third_size) : classified_by_everyone
                    + (i * third_size)
                ]
...
df_sorted.to_csv(
    config.train_image_sample_metadata_path.format(config.training_data_version),
    index=False,
)
