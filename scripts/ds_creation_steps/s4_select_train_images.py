...
def select_training_sample():
        ...
        # metadata["highway"] = metadata.highway.str.strip()
        ...
        with open(config.labeled_imgids_path, "r") as file:
            already_labled_ids = file.readlines()
        ...
        metadata = (
            ...
            .groupby(["tile_id"])  # restrict number of images per tile
        ...
        for surface in config.surfaces:
            for smoothness in config.surf_smooth_comb[surface]:
                ...
                metadata_fill_temp = (
                    metadata[~metadata.id.isin(metadata_temp.id)]
                    .loc[
                        (
                            (metadata["surface_clean"] == surface)
                            & (metadata["smoothness"] == smoothness),
        ...
        metadata_selection.to_csv(
            config.train_image_selection_metadata_path.format(config.training_data_versions),
            index=False,
        )
